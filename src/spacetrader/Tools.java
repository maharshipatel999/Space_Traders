...
public final class Tools {
     ...
     * based on the weights of each index as specified in the array probs. Specifically,
     * there is a (100 * probs[i] / total) percent likelihood of returning any index i between 0 and probs.length.
     * If total is greater than the sum of all the values in probs, then there is chance that
     * none of the indices in the array will be chosen, in that case -1 will be returned.
     * @param probs an array specifying the relative probability of each index
     * @param total the denominator of the probability of any number being chosen
     * (should be greater or equal to the sum of the values in probs)
     * @return an integer value between -1 (inclusive) and probs.length (exclusive)
     */
