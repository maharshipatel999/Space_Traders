...
public class Transaction {
    ...
    public void changeBuyQuantity(TradeGood good, int newQuantity) {
        ...
        int purchaseCost = (newQuantity - currQuantity) * market.getPrices().get(good);
        //Check if player has a large enough remaining balance
        if (remainingBalance < purchaseCost) {
            throw new IllegalArgumentException("Player does not have enough credits to make this purchase.");
        }
    ...
    public void changeSellQuantity(TradeGood good, int newQuantity) {
        ...
        if (newQuantity > playerCargo.getQuantityOfGood(good)) {
            throw new IllegalArgumentException("Cannot sell more goods than the player owns");
        }
    ...
    private void updateTotalCost() {
        ...
        for (TradeGood good : purchases.keySet()) {
            totalCost += purchases.get(good) * market.getPrices().get(good);
        }
        updateBalance();
    }
    ...
    private void updateTotalRevenue() {
        ...
        for (TradeGood good : sales.keySet()) {
            totalRevenue += sales.get(good) * 5; //market.getPrices().get(good);
        }
        updateBalance();
    }
